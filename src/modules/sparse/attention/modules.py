...
from .cross_windowed_attn import sparse_windowed_cross_attention
...
class SparseMultiHeadAttention(nn.Module):
    def __init__(
        ...
        assert type == "self" or attn_mode in ["full", "windowed"], (
            "Cross-attention supports full and windowed attention"
        )
    ...
    def forward(
        ...
        if self._type == "self":
            ...
            if (
                self.attn_mode == "windowed"
                and isinstance(x, SparseTensor)
                and isinstance(context, SparseTensor)
            ):
                k, v = kv.unbind(dim=1)
                h = sparse_windowed_cross_attention(
                    q,
                    k,
                    v,
