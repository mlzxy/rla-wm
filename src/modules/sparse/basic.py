...
class SparseTensor:
    ...
    def spatial_shape(self, value: List[int]):
        if BACKEND == "torchsparse":
            self.data.s = value
        elif BACKEND == "spconv":
            self.data.spatial_shape = value
        else:
            self.data["spatial_shape"] = value
    ...
    def occupancy(
        self, resolution: int | tuple[int, int, int] | None = None
    ) -> Float[Tensor, "B 1 D H W"]:
        out = self.replace(
            torch.ones(
                (self.feats.shape[0], 1), device=self.feats.device, dtype=torch.float32
            )
        )
        if resolution is not None:
            ...
            if BACKEND == "spconv":
                out.data.spatial_shape = resolution
                # out._shape = torch.Size([out.shape[0], 1] + resolution)
            else:
                out.data["spatial_shape"] = resolution
        return out.dense()
    ...
    def dense(self) -> torch.Tensor:
        if BACKEND == "torchsparse":
            ...
            ret = torch.zeros(
                *self.shape, *spatial_shape, dtype=self.dtype, device=self.device
            )
            ret[
                self.coords[:, 0],
                :,
    ...
    def replace(
        ...
        if BACKEND == "torchsparse":
            ...
            new_data = {
                ...
                if coords is None
                else list(coords.max(0)[0] + 1)[1:],
            }
        new_tensor = SparseTensor(
            new_data,
            shape=torch.Size(new_shape),
            layout=self.layout if coords is None else None,
            scale=self._scale,
