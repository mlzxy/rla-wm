...
class SubMConv3dFunction(Function):
    ...
    def _compute_neighbor_cache(
        ...
        if spconv.ALGORITHM in [
            ...
            if coords.is_cuda:
                neighbor_map = (
                    kernels.cuda.hashmap_build_submanifold_conv_neighbour_map_cuda(
                        hashmap_keys,
                        hashmap_vals,
                        coords,
                        W,
            ...
            gray_code, sorted_idx, valid_signal_i, valid_signal_o, valid_signal_seg = (
                kernels.cuda.neighbor_map_post_process_for_masked_implicit_gemm_1(
                    neighbor_map
                )
            )
    ...
    def _compute_neighbor_cache_torch(
        ...
        L = coords.shape[0]
        assert N * W * H * D <= 2**32, (
            "Currently, the max number of elements in a tensor is 2^32"
        )
        ...
        offset = torch.meshgrid(
            torch.arange(
                ...
                kernel_size[0] // 2 * dilation[0] + 1,
                dilation[0],
            ),
            torch.arange(
                -(kernel_size[1] // 2) * dilation[1],
                kernel_size[1] // 2 * dilation[1] + 1,
                dilation[1],
            ),
            torch.arange(
                -(kernel_size[2] // 2) * dilation[2],
                kernel_size[2] // 2 * dilation[2] + 1,
                dilation[2],
            ),
            indexing="ij",
        )
        offset = torch.stack(offset, dim=-1).reshape(-1, 3).int().to(coords.device)
        neighbor_coords = coords.unsqueeze(1).repeat(
        ...
        neighbor_search_indices = torch.clamp(
            neighbor_search_indices, 0, sorted_keys.shape[0] - 1
        )
        neighbor_valid &= sorted_keys[neighbor_search_indices] == neighbor_keys
        neighbor_indices = torch.full(
            (L * kernel_size[0] * kernel_size[1] * kernel_size[2],),
            0xFFFFFFFF,
            dtype=torch.long,
            device=coords.device,
        )
        neighbor_indices[neighbor_valid] = indices[
            neighbor_search_indices[neighbor_valid]
        ]
        return SubMConv3dNeighborCache(
            **{"neighbor_map": neighbor_indices.reshape(L, -1).to(torch.uint32)}
        )
    ...
    def _sparse_submanifold_conv_forward(
        ...
        if spconv.ALGORITHM == Algorithm.EXPLICIT_GEMM:
            ...
            output = (
                kernels.triton.sparse_submanifold_conv_fwd_masked_implicit_gemm_splitk(
                    feats,
                    weight.reshape(Co, Kd * Kh * Kw, Ci),
                    bias,
                    neighbor_cache["neighbor_map"],
    ...
    def _sparse_submanifold_conv_backward(
        ...
        if spconv.ALGORITHM == Algorithm.EXPLICIT_GEMM:
            ...
            if feats.requires_grad:
                # im2col
                im2col = torch.zeros(
                    (N * V, Co), device=feats.device, dtype=feats.dtype
                )
                ...
                # addmm
                grad_input = torch.mm(
                    im2col, weight.view(Co, V, Ci).transpose(0, 1).reshape(V * Co, Ci)
                )
            else:
                grad_input = None
            ...
            if weight.requires_grad:
                # im2col
                im2col = torch.zeros(
                    (N * V, Ci), device=weight.device, dtype=weight.dtype
                )
                mask = neighbor_map.view(-1) != 0xFFFFFFFF
                im2col[mask] = feats[neighbor_map.view(-1).long()[mask]]
                im2col = im2col.view(N, V * Ci)
                ...
                # addmm
                grad_weight = (
                    torch.mm(im2col.t(), grad_output.view(N, -1))
                    .view(V, Ci, Co)
                    .permute(2, 0, 1)
                    .contiguous()
                    .view(Co, Kw, Kh, Kd, Ci)
            ...
            grad_input, grad_weight, grad_bias = (
                kernels.triton.sparse_submanifold_conv_bwd_implicit_gemm(
            ...
            grad_input, grad_weight, grad_bias = (
                kernels.triton.sparse_submanifold_conv_bwd_implicit_gemm_splitk(
                    grad_output.contiguous(),
                    feats,
                    weight.reshape(Co, Kd * Kh * Kw, Ci),
                    bias,
            ...
            grad_input, grad_weight, grad_bias = (
                kernels.triton.sparse_submanifold_conv_bwd_masked_implicit_gemm(
                    grad_output.contiguous(),
                    feats,
                    weight.reshape(Co, Kd * Kh * Kw, Ci),
                    bias,
            ...
            grad_input, grad_weight, grad_bias = (
                kernels.triton.sparse_submanifold_conv_bwd_masked_implicit_gemm_splitk(
                    grad_output.contiguous(),
                    feats,
                    weight.reshape(Co, Kd * Kh * Kw, Ci),
                    bias,
    ...
    def forward(
        ...
        if neighbor_cache is None:
            neighbor_cache = SubMConv3dFunction._compute_neighbor_cache(
                coords, shape, (Kw, Kh, Kd), dilation
            )

        # compute output
        output = SubMConv3dFunction._sparse_submanifold_conv_forward(
