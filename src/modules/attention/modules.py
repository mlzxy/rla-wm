    ...
    def __init__(
        ...
        if use_rope:
            self.rope = RotaryPositionEmbedder(
                self.head_dim, in_channels=rope_in_channels
            )
    ...
    def forward(
        ...
        if self._type == "self":
            ...
            if self.use_rope:
                ...
                if indices is None:
                    # Generate indices for sequence dim (1)
                    # q: (B, L, H, D)
                    seq_len = q.shape[1]
                    indices = torch.arange(seq_len, device=q.device)
                    # Reshape to (1, L, 1, 1) for broadcasting: B, L, 1, C=1
                    indices = indices.reshape(1, seq_len, 1, 1).expand(
                        q.shape[0], seq_len, 1, 1
                    )
            ...
            if self.attn_mode == "full":
                if self.qk_rms_norm:
                    ...
                    h = scaled_dot_product_attention(qkv, attn_mask=attn_mask)
