...
1. Global Balancing (BalancedResumableSampler): Distributes data across GPUs to ensure equal
    total voxel load per GPU, while maintaining identical batch counts (equal_size=True).
2. Local Balancing (collate_fn): Re-groups the assigned batch into micro-batches (split_size) to
    smooth memory usage. This step allows unequal item counts (equal_size=False) to perfectly
        balance computational load across accumulation steps.
"""
...
class SparseFlowMatchingTrainer(FlowMatchingTrainer):
    ...
    def training_losses(
        self, x_0: sp.SparseTensor, cond=None, **kwargs
    ) -> Tuple[Dict, Dict]:
        """
...
class SparseFlowMatchingCFGTrainer(
    ClassifierFreeGuidanceMixin, SparseFlowMatchingTrainer
):
    """
...
class TextConditionedSparseFlowMatchingCFGTrainer(
...
class ImageConditionedSparseFlowMatchingCFGTrainer(
    ImageConditionedMixin, SparseFlowMatchingCFGTrainer
):
    """
