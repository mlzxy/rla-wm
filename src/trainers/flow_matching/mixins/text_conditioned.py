...
class TextConditionedMixin:
    ...
    def __init__(
        self, *args, text_cond_model: str = "openai/clip-vit-large-patch14", **kwargs
    ):
    ...
    @torch.no_grad()
    def encode_text(self, text: List[str]) -> Float[Tensor, "B L D"]:
        """
        Encode the text.
        ...
        Returns:
            Tensor of shape [B, L, D], where B is the batch size, L is the sequence length, and D is the text embedding dimension.
        """
    ...
    def get_cond(self, cond: List[str], **kwargs) -> Float[Tensor, "B L D"]:
        """
        Get the conditioning data.

        Args:
            cond: List of text strings.
        ...
        kwargs["neg_cond"] = self.text_cond_model["null_cond"].repeat(
            cond.shape[0], 1, 1
        )
        cond = super().get_cond(cond, **kwargs)  # classifier free guidance
        return cond

    def get_inference_cond(self, cond: List[str], **kwargs) -> Float[Tensor, "B L D"]:
        """
