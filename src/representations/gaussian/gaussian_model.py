...
import utils3d
from utils.render import SplatsDict
...
class Gaussian:
    ...
    def to_splats_dict(self) -> SplatsDict:
        # SH constant
        C0 = 0.28209479177387814

        # colors: SH -> RGB
        # _features_dc is [N, 1, 3] -> [N, 3]
        colors = self._features_dc.squeeze(1) * C0 + 0.5
        # colors = torch.clamp(colors, min=0.0, max=1.0)

        ret = {
            "means": self.get_xyz.float(),
            "quats": self.get_rotation.float(),  # Already in wxyz format
            "scales": self.get_scaling.float(),
            "opacities": self.get_opacity.squeeze(-1).float(),
            "colors": colors.float(),
        }
        
        if self.get_mask is not None:
            ret["masks"] = self.get_mask.float()
    ...
    def from_pcds(
        ...
        num_batches: int,
        aabb: list[float] = [0, 0, 0, 1.0, 1.0, 1.0],
        ...
        for b in range(num_batches):
            ...
            if pts.shape[0] == 0:
                gaussian._xyz = torch.zeros(0, 3, device=device, dtype=dtype)
                gaussian._features_dc = torch.zeros(0, 3, 1, device=device, dtype=dtype)
                gaussian._scaling = torch.zeros(0, 3, device=device, dtype=dtype)
                gaussian._rotation = torch.zeros(0, 4, device=device, dtype=dtype)
                gaussian._opacity = torch.zeros(0, 1, device=device, dtype=dtype)
            else:
                gaussian._xyz = pts
                # RGB -> SH
                shs = (rgbs - 0.5) / C0
                gaussian._features_dc = shs.unsqueeze(1)  # [N, 1, 3]

                gaussian.from_scaling(
                    torch.full(
                        (pts.shape[0], 3), point_size, device=device, dtype=dtype
                    )
                )
                ...
                gaussian.from_opacity(
                    torch.ones(pts.shape[0], 1, device=device, dtype=dtype)
                )
