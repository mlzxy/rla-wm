...
class SLatGaussianDecoder(SparseTransformerBase):
    ...
    def _build_perturbation(self) -> None:
        ...
        perturbation = (
            torch.tensor(perturbation).float() * 2 - 1
        )  # num_gaussians -> 32 (inside the one unit!)
    ...
    def to_representation(
        self, x: sp.SparseTensor, aabb=[-0.5, -0.5, -0.5, 1.0, 1.0, 1.0]
    ) -> list[Gaussian]:
        """
    ...
    def forward(self, x: sp.SparseTensor, to_rep_kwargs={}) -> list[Gaussian]:
        h = super().forward(x)  # shape (1, 8)
        h = h.type(x.dtype)
        h = h.replace(F.layer_norm(h.feats, h.feats.shape[-1:]))
        h = self.out_layer(h)
        return self.to_representation(h, **to_rep_kwargs)
