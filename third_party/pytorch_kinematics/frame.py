...
from third_party.pytorch_kinematics.transforms import axis_and_angle_to_matrix_33
...
class Link(object):
    ...
    def to(self, *args, **kwargs):
        ...
        # Move any attached geometry transforms to the requested device/dtype
        for visual in self.visuals:
            if visual.offset is not None:
                visual.offset = visual.offset.to(*args, **kwargs)
        for collision in self.collisions:
            if collision.offset is not None:
                collision.offset = collision.offset.to(*args, **kwargs)
        return self

    def __repr__(self):
        return "Link(name='{0}', offset={1}, visuals={2}, collisions={3})".format(
            self.name, self.offset, self.visuals, self.collisions)
