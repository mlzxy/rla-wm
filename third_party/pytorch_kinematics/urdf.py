...
from . import chain
import torch
import third_party.pytorch_kinematics.transforms as tf
...
def _build_chain_recurse(root_frame, lmap, joints):
    ...
    for j in joints:
        if j.parent == root_frame.link.name:
            ...
            link = lmap[j.child]
            # Convert all visuals (not just the first one)
            converted_visuals = [_convert_visual(vis) for vis in link.visuals] if link.visuals else []
            converted_collisions = [_convert_visual(col) for col in link.collisions] if link.collisions else []
            child_frame.link = frame.Link(link.name, offset=_convert_transform(link.origin),
                                          visuals=converted_visuals,
...
def build_chain_from_urdf(data):
    ...
    # Convert all visuals (not just the first one)
    converted_visuals = [_convert_visual(vis) for vis in root_link.visuals] if root_link.visuals else []
    converted_collisions = [_convert_visual(col) for col in root_link.collisions] if root_link.collisions else []
    root_frame.link = frame.Link(root_link.name, _convert_transform(root_link.origin),
                                 converted_visuals,
                                 converted_collisions)
