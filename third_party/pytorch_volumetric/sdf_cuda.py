...
from .voxel import (
    VoxelGrid,
    get_divisible_range_by_resolution,
...
class ObjectFactory(abc.ABC):
    ...
    def __reduce__(self):
        return partial(
            self.__class__,
            scale=self.scale,
            vis_frame_pos=self.vis_frame_pos,
            vis_frame_rot=self.vis_frame_rot,
            plausible_suboptimality=self.plausible_suboptimality,
            **self.other_load_kwargs,
        ), (self.name,)
    ...
    def draw_mesh(self, dd, name, pose, rgba, object_id=None):
        ...
        return dd.draw_mesh(
            name,
            self.get_mesh_resource_filename(),
            pose,
            scale=self.scale,
            rgba=rgba,
            object_id=object_id,
            vis_frame_pos=frame_pos,
            vis_frame_rot=self.vis_frame_rot,
        )
    ...
    def precompute_sdf(self):
        if self._mesh is None:
            ...
            self._mesh = self._mesh.rotate(
                o3d.geometry.get_rotation_matrix_from_quaternion((w, x, y, z)),
                center=[0, 0, 0],
            )
            self._mesh = self._mesh.translate(np.array(self.vis_frame_pos) * self.scale)

        # Prepare Kaolin data structures on GPU
        # Check if already computed
        if not hasattr(self, "face_verts_cuda") or self.face_verts_cuda is None:
            # Vertices: (V, 3)
            self.verts_cuda = (
                torch.from_numpy(np.asarray(self._mesh.vertices)).float().cuda()
            )
            # Faces: (F, 3)
            self.faces_cuda = (
                torch.from_numpy(np.asarray(self._mesh.triangles)).long().cuda()
            )

            # Precompute face vertices for distance query (1, F, 3, 3)
            # Index vertices by faces: (F, 3, 3)
            # We unsqueeze to add batch dim (1, F, 3, 3)
            face_verts = kaolin.ops.mesh.index_vertices_by_faces(
                self.verts_cuda.unsqueeze(0), self.faces_cuda
            )
            self.face_verts_cuda = face_verts

            # Precompute face normals on GPU
            if self._face_normals is None:
                self._mesh.compute_triangle_normals()
                self._face_normals = np.asarray(self._mesh.triangle_normals)
            self.face_normals_cuda = torch.from_numpy(self._face_normals).float().cuda()

    # @tensor_utils.handle_batch_input(n=2)
    def _do_object_frame_closest_point(
        self, points_in_object_frame, compute_normal=False
    ):
        """
        Implementation using Kaolin on GPU
        """
        assert torch.is_tensor(points_in_object_frame) or isinstance(
            points_in_object_frame, np.ndarray
        )

        # Ensure input is torch tensor
        if not torch.is_tensor(points_in_object_frame):
            points = torch.tensor(
                points_in_object_frame, dtype=torch.float32, device="cuda"
            )
        else:
            # We work on a detached version to avoid building a graph
            points = points_in_object_frame
            # .to(
            #     device="cuda", dtype=torch.float32
            # ).detach()

        # Handle batching manually since the decorator seems to fail in some environments
        ...
        closest, sdf_dist, grad = compute_sdf(
            points_batch,
            self.verts_cuda.unsqueeze(0),
            self.faces_cuda,
        ...
        if len(original_shape) > 2:
            ...
            # dist is scalar per point
            output_dist = sdf_dist.view(*batch_dims)
            # grad/closest are vector per point
            output_grad = grad.view(*batch_dims, 3)
            output_closest = closest.view(*batch_dims, 3)
        else:
            output_dist = sdf_dist.squeeze(0)  # (N,)
            output_grad = grad.squeeze(0)  # (N, 3)
            output_closest = closest.squeeze(0)  # (N, 3)

        output_normals = None

        return output_closest, output_dist, output_grad, output_normals

    def object_frame_closest_point(
        self, points_in_object_frame, compute_normal=False
    ) -> SDFQuery:
        """
        ...
        return SDFQuery(
            *self._do_object_frame_closest_point(
                points_in_object_frame, compute_normal=compute_normal
            )
        )
...
class ObjectFrameSDF(Function):
    ...
    @abc.abstractmethod
    def surface_bounding_box(self, padding=0.0, padding_ratio=0.0):
        """
    ...
    def get_voxel_view(
        self, voxels: VoxelGrid = None, dtype=torch.float, device="cpu"
    ) -> torch_view.TorchMultidimView:
        """
        ...
        if voxels is None:
            voxels = VoxelGrid(
                0.01,
                self.surface_bounding_box(padding=0.1).cpu().numpy(),
                dtype=dtype,
                device=device,
            )
        ...
        return torch_view.TorchMultidimView(
            cached_underlying_sdf, voxels.range_per_dim, invalid_value=self.__call__
        )

    def get_filtered_points(
        self, unary_filter, voxels: VoxelGrid = None, dtype=torch.float, device="cpu"
    ) -> torch.tensor:
        """
...
class CylinderSDF(ObjectFrameSDF):
    """SDF for a geometric primitive, a cylinder aligned with the z-axis, centered at origin.
    The cylinder extends from -length/2 to +length/2 along z-axis."""
    ...
    def __call__(self, points_in_object_frame):
        return self.apply(points_in_object_frame, self.radius, self.length)

    @staticmethod
    def forward(ctx, points_in_object_frame, radius, length):
        # Points are in object frame, cylinder is aligned with z-axis
        # Project to xy plane for radial distance
        xy_dist = torch.linalg.norm(points_in_object_frame[..., :2], dim=-1)
        z_coord = points_in_object_frame[..., 2]

        # Distance to cylinder surface
        dist_to_cylinder = xy_dist - radius

        # Distance to top/bottom caps
        half_length = length / 2.0
        dist_to_top = half_length - z_coord
        dist_to_bottom = z_coord + half_length

        # For points outside the cylinder radius
        outside_radius = dist_to_cylinder > 0
        # For points above the top cap
        above_top = z_coord > half_length
        # For points below the bottom cap
        below_bottom = z_coord < -half_length

        # Compute SDF value
        # If outside radius and within z bounds, use cylinder distance
        # If within radius but outside z bounds, use cap distance
        # If outside both, use combined distance
        dist = torch.zeros_like(xy_dist)
        grad = torch.zeros_like(points_in_object_frame)

        # Case 1: Within z bounds, outside radius
        case1 = outside_radius & (~above_top) & (~below_bottom)
        dist[case1] = dist_to_cylinder[case1]
        grad[case1, :2] = points_in_object_frame[case1, :2] / (
            xy_dist[case1].unsqueeze(-1) + 1e-12
        )
        grad[case1, 2] = 0

        # Case 2: Within radius, above top
        case2 = (~outside_radius) & above_top
        dist[case2] = dist_to_top[case2]
        grad[case2, :2] = 0
        grad[case2, 2] = -1

        # Case 3: Within radius, below bottom
        case3 = (~outside_radius) & below_bottom
        dist[case3] = dist_to_bottom[case3]
        grad[case3, :2] = 0
        grad[case3, 2] = 1

        # Case 4: Outside radius and above top
        case4 = outside_radius & above_top
        corner_dist = torch.sqrt(dist_to_cylinder[case4] ** 2 + dist_to_top[case4] ** 2)
        dist[case4] = corner_dist
        grad[case4, :2] = (
            (dist_to_cylinder[case4] / corner_dist).unsqueeze(-1)
            * points_in_object_frame[case4, :2]
            / (xy_dist[case4].unsqueeze(-1) + 1e-12)
        )
        grad[case4, 2] = -dist_to_top[case4] / corner_dist

        # Case 5: Outside radius and below bottom
        case5 = outside_radius & below_bottom
        corner_dist = torch.sqrt(
            dist_to_cylinder[case5] ** 2 + dist_to_bottom[case5] ** 2
        )
        dist[case5] = corner_dist
        grad[case5, :2] = (
            (dist_to_cylinder[case5] / corner_dist).unsqueeze(-1)
            * points_in_object_frame[case5, :2]
            / (xy_dist[case5].unsqueeze(-1) + 1e-12)
        )
        grad[case5, 2] = dist_to_bottom[case5] / corner_dist
    ...
    def surface_bounding_box(self, padding=0.0, padding_ratio=0.0):
        r = self.radius + padding + padding_ratio * self.radius
        half_l = self.length / 2.0 + padding + padding_ratio * self.length / 2.0
        return torch.tensor([[-r, r], [-r, r], [-half_l, half_l]])


class CylinderObjectFactory(ObjectFactory):
    """Factory for creating cylinder meshes from primitive geometry."""

    def __init__(self, radius, length, resolution=20, **kwargs):
        self.radius = radius
        self.length = length
        ...
        super(CylinderObjectFactory, self).__init__(mesh=mesh, **kwargs)
    ...
    def get_mesh_resource_filename(self):
        # No file, this is a primitive
        return None


class SphereObjectFactory(ObjectFactory):
    """Factory for creating sphere meshes from primitive geometry."""

    def __init__(self, radius, resolution=20, **kwargs):
        self.radius = radius
        self.resolution = resolution
        # Create mesh immediately
        ...
        super(SphereObjectFactory, self).__init__(mesh=mesh, **kwargs)
...
class BoxObjectFactory(ObjectFactory):
    """Factory for creating box meshes from primitive geometry."""
    ...
    def __init__(self, size, **kwargs):
        ...
        # Center the box
        mesh.translate(-np.array(size) / 2.0)
        super(BoxObjectFactory, self).__init__(mesh=mesh, **kwargs)
...
class ComposedSDF(ObjectFrameSDF):
    def __init__(
        self,
        sdfs: typing.Sequence[ObjectFrameSDF],
        obj_frame_to_each_frame: pk.Transform3d = None,
    ):
        """
        ...
        self.link_frame_to_obj_frame: typing.Optional[
            typing.Sequence[pk.Transform3d]
        ] = None
        self.tsf_batch = None
    ...
    def forward(
        ...
        points_in_object_frame,
        sdfs,
        obj_frame_to_link_frame,
        tsf_batch,
        link_frame_to_obj_frame,
    ):
...
class CachedSDF(ObjectFrameSDF):
    ...
    def __init__(
        ...
        if min(num_voxel) < 10:
            logger.warning(
                f"Resolution {resolution} is too high for {object_name}, only getting {num_voxel} voxels."
            )
        ...
        if os.path.exists(cache_path):
            ...
            try:
                ...
                logger.info(
                    "cached sdf invalid %s from %s, recreating", self.name, cache_path
                )
        else:
            data = {}
        ...
        if cached_underlying_sdf is None or clean_cache:
            if gt_sdf is None:
                raise RuntimeError(
                    "Cached SDF did not find the cache and requires an initialize queryable SDF"
                )

            coords, pts = get_coordinates_and_points_in_grid(
                self.resolution, self.ranges
            )
            ...
            if self.debug_check_sdf:
                debug_view = torch_view.TorchMultidimView(
                    cached_underlying_sdf,
                    self.ranges,
        ...
        self.voxels = torch_view.TorchMultidimView(
            cached_underlying_sdf,
            range_per_dim,
            invalid_value=self._fallback_sdf_value_func,
        )
    ...
    def outside_surface(self, points_in_object_frame, surface_level=0):
        ...
        outside[inbound_keys] = (
            self.voxels.raw_data[keys_ravelled[inbound_keys]] > surface_level
        )
        return outside
