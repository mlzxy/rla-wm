...
class ObjectFactory(abc.ABC):
    ...
    def precompute_sdf(self):
        ...
        if self._mesht is None:
            # Create a clean mesh from vertices and triangles to avoid Open3D warning/data loss
            # when converting multi-material legacy meshes to tensor meshes.
            clean_mesh = o3d.geometry.TriangleMesh()
            clean_mesh.vertices = self._mesh.vertices
            clean_mesh.triangles = self._mesh.triangles

            self._mesht = o3d.t.geometry.TriangleMesh.from_legacy(clean_mesh)
    ...
    def _do_object_frame_closest_point(
        ...
        gradient[has_direction] = (
            gradient[has_direction] / distance[has_direction, None]
        )

        # ensure ray destination is outside the object
        ray_destination = np.repeat(
            self.bounding_box(padding=1.0)[None, :, 1],
            points_in_object_frame.shape[0],
            axis=0,
        )
        # add noise to ray destination, this helps reduce artifacts in the sdf
        ray_destination = ray_destination + 1e-4 * np.random.randn(
            *points_in_object_frame.shape
        )
        ...
        pts, distance, gradient = tensor_utils.ensure_tensor(
            device, dtype, pts, distance, gradient
        )
...
def sample_mesh_points(
    ...
    if cache is not None or os.path.exists(dbpath):
        ...
        if not clean_cache and num_points in cache[name][seed]:
            ...
            res = list(
                v.to(device=device, dtype=dtype) if v is not None else None for v in res
            )
            return *res[:-1], cache
    else:
        cache = {name: {seed: {}}}
    ...
    if obj_factory is None:
        raise RuntimeError(
            f"Expect model points to be cached for {name} {seed} {num_points} in {dbpath}"
        )
    ...
    return (
        points.to(device=device, dtype=dtype),
