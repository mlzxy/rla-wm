...
from third_party.pytorch_kinematics import transforms as tf
from third_party.pytorch_kinematics.transforms.rotation_conversions import matrix_to_pos_rot

from .sdf import ObjectFactory, sample_mesh_points, ObjectFrameSDF


def pairwise_distance(world_to_link_tfs: pk.Transform3d):
    m = world_to_link_tfs.get_matrix()
    t = m[:, :3, 3]
    r = tf.rotation_conversions.matrix_to_rotation_6d(m[:, :3, :3])
