...
for seed in $@; do
    ...
    .venv/bin/python wmrl/train.py --tag "${task}-apr22-${rewardshortname}-lora-seed${seed}" --env-kwargs.reward-mode ${reward} --env-kwargs.terminal-success-bonus 0.0 --config-file wmrl/configs/${task}.yaml --total-iterations 50 --run-initial-eval --bc-loss-weight 0.0 --num-envs 32  --num-steps 4  --mini-batch-size 64 --seed $seed  --use-critic --policy-kwargs.enable-rl-lora  --env-batch-num 4 # --no-use-wandb
done
