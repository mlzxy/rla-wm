...
from datalib.src.tasks import *
from datalib.src.robots import *
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.save_model and iteration % args.eval_freq == 1:
            model_path = f"runs/PPO/{run_name}/ckpt_{iteration}.pt"
        ...
        with torch.no_grad():
            ...
            lastgaelam = 0
            for t in reversed(range(args.num_steps)):  #! 反向遍历方便!
                ...
                if args.finite_horizon_gae:
                    ...
                    # NOTE: single step TD-error
                    delta = rewards[t] + args.gamma * real_next_values - values[t]
                    # NOTE: is the discounted sum of TD-errors
                    ...
                    # NOTE: lastgaelam is the advantage of the previous step
            returns = advantages + values  # \hat{R}_t = Q(s, a) = A(s, a) + V(s)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # NOTE: if this action is more likely, then logratio is positive
                # NOTE: then it will has a larger impact on loss (self-reinforcing, so we have the clipping!)
                ratio = logratio.exp()  # NOTE: likelihood ratio!
                ...
                if args.norm_adv:  # by default yes
    ...
    if not args.evaluate:
        if args.save_model:
            model_path = f"runs/PPO/{run_name}/final_ckpt.pt"
            latest_path = f"runs/PPO/{run_name}/latest.pt"
            torch.save(agent.state_dict(), model_path)
            torch.save(agent.state_dict(), latest_path)
            print(f"model saved to {model_path} and {latest_path}")
