...
def _get_max_episode_steps_from_temp_env(env_id: str, env_kwargs: dict) -> Optional[int]:
    """Create a temporary single env to read max_episode_steps, then close it."""
    try:
        temp_env = gym.make(env_id, num_envs=1, **env_kwargs)
        try:
            return gym_utils.find_max_episode_steps_value(temp_env)
        finally:
            temp_env.close()
    except Exception:
        return None
...
class Args:
    ...
    cpu_training: bool = True
    """if toggled, run training envs via CPUGymWrapper vector envs (numpy outputs converted to tensors on device)"""
...
def train(args=None, callback=None, progress: Progress = None, task_id: TaskID = None):
    ...
    eval_success = 0.0
    use_cpu_train = args.cpu_training and not args.evaluate
    # env setup
    env_kwargs = dict(
        obs_mode="state",
    ...
    if env_kwargs["sim_backend"] == "physx_cpu" and not use_cpu_train:
        args.num_envs = 1
    ...
    train_env_kwargs = {**env_kwargs, "sim_backend": "physx_cpu"}
    ...
    use_cpu_eval = True
    use_async_train = args.num_envs > 1
    ...
    def cpu_make_env(eval_env: bool = True, render_mode=None):
        ...
        def thunk():
            ...
            kw = dict(eval_env_kwargs if eval_env else train_env_kwargs)
            kw["num_envs"] = 1
            if render_mode:
    ...
    if use_cpu_train:
        logger.log(
            f"Using {'Sync' if args.num_envs == 1 else 'Async'}VectorEnv for training with num_envs={args.num_envs} (physx_cpu, CPUGymWrapper)"
        )
        train_fns = [cpu_make_env(eval_env=False) for _ in range(args.num_envs)]
        envs = vector_cls_train(train_fns)
    else:
        envs = gym.make(
            args.env_id,
    ...
    if args.capture_video:
        ...
        if args.save_train_video_freq is not None and not use_cpu_train:
        ...
        elif args.save_train_video_freq is not None and use_cpu_train:
            logger.log("Skipping training video capture for cpu_training envs.")
        if eval_envs is not None:
    ...
    if use_cpu_train:
        max_episode_steps = _get_max_episode_steps_from_temp_env(args.env_id, train_env_kwargs)
        if max_episode_steps is None:
            max_episode_steps = 200
        logger.log(f"max_episode_steps (from temp env): {max_episode_steps}")
    else:
    ...
    if use_cpu_train:
        next_obs = torch.as_tensor(np.asarray(next_obs), device=device, dtype=torch.float32)
    eval_obs = None
    if eval_envs is not None:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            env_action = clip_action(action)
            if use_cpu_train and torch.is_tensor(env_action):
            ...
            if use_cpu_train:
                next_obs = torch.as_tensor(np.asarray(next_obs), device=device, dtype=torch.float32)
                reward = torch.as_tensor(np.asarray(reward), device=device, dtype=torch.float32)
                terminations = torch.as_tensor(np.asarray(terminations), device=device)
                truncations = torch.as_tensor(np.asarray(truncations), device=device)
            ...
            if "final_info" in infos:
                ...
                done_mask_np = (
                    np.asarray(done_mask, dtype=bool) if use_cpu_train else None
                )
                if use_cpu_train:
                    _latest_metrics = defaultdict(list)
                    for vv in final_info[done_mask_np]:
                        _latest_metrics['success'].append(vv['success'])
                        _latest_metrics['reward'].append(vv['episode']['reward'])
                        _latest_metrics['return'].append(vv['episode']['return'])
                    for k, v in _latest_metrics.items():
                        val = np.mean(v)
                ...
                with torch.no_grad():
                    if use_cpu_train:
                        done_indices_np = np.nonzero(done_mask_np)[0]
                        if done_indices_np.size > 0:
                            # For CPU vector env final observations, index with numpy mask first,
                            # then convert to list before creating a tensor on the model device.
                            sampled_final_obs = np.asarray(
                                infos["final_observation"], dtype=object
                            )[done_mask_np].tolist()
                            final_obs_tensor = torch.as_tensor(
                                sampled_final_obs, device=device, dtype=next_obs.dtype
                            )
                            done_indices = torch.as_tensor(
                                done_indices_np, device=device, dtype=torch.long
                            )
                            final_values[step, done_indices] = get_value(
                                final_obs_tensor
                            ).view(-1)
                    else:
                        final_values[
